...
# 2. eleva ao quadrado (pixel a pixel)
pc_quad = pc_median ** 2

# 3. min-max -> [0, 127], arredondado a inteiro
qmin, qmax = pc_quad.min(), pc_quad.max()
mediana_quad127 = np.round((pc_quad - qmin) / (qmax - qmin) * LIM).astype(np.int32)
superficie = (mediana_quad127 / LIM).astype(np.float32)

# 4. Otsu adaptativo + contornos
otsu_nivel = threshold_otsu(mediana_quad127.astype(np.uint16))
...
print(f"mediana^2: min={qmin:.1f} max={qmax:.1f}")
...
axes[0].imshow(mediana_quad127, cmap="gray", vmin=0, vmax=LIM)
axes[0].set_title(f"Mediana² min-max → [0,{LIM}]")
axes[0].axis("off")

axes[1].hist(mediana_quad127.ravel(), bins=128, color='gray', edgecolor='black')
axes[1].axvline(otsu_nivel, color='red', linewidth=2, label=f'Otsu = {otsu_nivel}')
axes[1].set_title("Histograma mediana² [0,127] + limiar Otsu")
axes[1].set_xlabel("nível")
axes[1].legend()

axes[2].imshow(mediana_quad127, cmap="gray", vmin=0, vmax=LIM)
...
plt.tight_layout()
out_path = "/home/cuco/projetos/casaiq/mediana_quad127_otsu.png"
